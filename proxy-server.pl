	...
	request => sub {
		...
		if ($method eq 'OPTIONS') {
			...
			if ($path =~ /^\/cpl/) {
				...
				my $has_memory = 0;
				my $json_error = 0;    #$util->eval_json(str => $req_body);
				...
				if (!$has_memory) {
					send_response(headers_in => $headers_in, method => $method, body => $req_body, req => $req, up => $up, json_error => $json_error);
				}
			}
		}
	}
...
sub send_response {
	...
	http_request $method => "$up",
	  %client_options, sub {
		...
		if ($status == 200 && !$json_error) {
			$memory_idx++;

			# Decode UTF-8 bytes for proper string handling
			my $decoded_req_body      = eval { decode_utf8($req_body,      Encode::FB_CROAK) } || $req_body;
			my $decoded_response_body = eval { decode_utf8($response_body, Encode::FB_CROAK) } || $response_body;
